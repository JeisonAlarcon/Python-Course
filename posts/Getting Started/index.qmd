...
```{python}
# Argumentos y Parámetros Arbitrarios:
def sumar(*args):
    return sum(args)
...
Características: - Permite pasar una cantidad variable de argumentos a una función. - Los argumentos se agrupan en una tupla dentro de la función.
...
Características: - Permite pasar una cantidad variable de argumentos con nombre a una función. - Los argumentos se agrupan en un diccionario dentro de la función.
...
Otros comandos útiles en Python

sys.argv

El módulo sys proporciona acceso a algunos objetos utilizados o mantenidos por el intérprete de Python y a funciones que interaccionan fuertemente con el intérprete. sys.argv es una lista en la que cada elemento es una cadena de caracteres. Contiene los argumentos pasados al script.
...
El módulo os permite interactuar con el sistema operativo. os.environ es un diccionario que contiene las variables de entorno del sistema.
